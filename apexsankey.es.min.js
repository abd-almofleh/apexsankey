...
class Paper {
  constructor(element, width2, height2, canvasStyle, spacing) {
    ...
    this.canvas = SVG().addTo(element).size("100%", "100%").viewbox(`-${spacing} -${spacing} ${width2 + spacing * 2} ${height2 + spacing}`).attr({ style: canvasStyle });
  }
  add(element) {
...
function renderNode(graphNode, maxRank, options) {
  ...
  const { nodeWidth, nodeBorderWidth, nodeBorderColor, onNodeClick, fontSize, fontColor } = options;
  ...
  group.add(nodeBody);
  console.log("node", graphNode.data.title, dy2, Number.parseInt(fontSize));
  if (graphNode.data && dy2 > Number.parseInt(fontSize)) {
...
class SankeyGraph extends Paper {
  ...
  construct(data2) {
    const { viewPortWidth, viewPortHeight, spacing, nodeWidth } = this.options;
  ...
  renderEdge(edgeObj, group) {
    ...
    if (csNode.dummy) {
      edgeLabel.x0 = edgeLabel.x0 - nodeWidth / 2;
    }
    if (ctNode.dummy) {
      edgeLabel.x1 = edgeLabel.x1 + nodeWidth / 2;
    }
